    ...
    if (checksheetWizardRoot) {
        ...
        const renderMachineList = () => {
            ...
            machineList.innerHTML = filteredMachines().map((machine) => {
                ...
                return `<label class="flex cursor-pointer items-center justify-between gap-3 border-b border-[var(--color-prime-border)] px-4 py-3 last:border-b-0"><span class="inline-flex items-center gap-3"><input type="checkbox" value="${machine.id}" ${checked} data-machine-checkbox><span class="font-semibold">${machine.code}</span><span class="text-[var(--color-prime-muted)]">${machine.name}</span></span><span class="rounded-full bg-[var(--color-prime-soft)] px-2 py-1 text-xs text-[var(--color-prime-muted)]">${machine.location ?? '-'}</span></label>`;
            }).join('');
        };
        ...
        const renderParts = () => {
            ...
            partsBuilder.innerHTML = state.selectedMachineIds.map((machineId) => {
                ...
                return `<div class="rounded-xl border border-[var(--color-prime-border)] p-4"><div class="mb-3 flex items-center justify-between"><h4 class="font-semibold">${machine?.code} - ${machine?.name}</h4><button type="button" class="text-sm font-semibold text-[var(--color-prime-primary)]" data-add-part="${machineId}">+ Tambah Part</button></div>${parts.map((part, index) => `<div class="mb-2 grid gap-2 md:grid-cols-[3rem_minmax(0,1fr)_minmax(0,1fr)_2rem]"><div class="rounded-lg bg-[var(--color-prime-soft)] px-3 py-2 text-center text-sm">${index + 1}</div><input type="text" value="${part.name ?? ''}" placeholder="Nama Part" class="rounded-lg border border-[var(--color-prime-border)] px-3 py-2" data-part-name="${machineId}:${part.id}"><input type="text" value="${part.description ?? ''}" placeholder="Deskripsi opsional" class="rounded-lg border border-[var(--color-prime-border)] px-3 py-2" data-part-description="${machineId}:${part.id}"><button type="button" class="text-[var(--color-prime-danger)]" data-remove-part="${machineId}:${part.id}">x</button></div>`).join('')}</div>`;
            }).join('');
        };
        ...
        const renderStandards = () => {
            ...
            state.selectedMachineIds.forEach((machineId) => {
                ...
                (state.parts[machineId] ?? []).forEach((part) => {
                    ...
                    blocks.push(`<div class="rounded-xl border border-[var(--color-prime-border)] p-4"><div class="mb-3 flex items-center justify-between"><div><p class="text-sm text-[var(--color-prime-muted)]">${machine?.code}</p><h4 class="font-semibold">${part.name || '(Part belum diisi)'}</h4></div><button type="button" class="text-sm font-semibold text-[var(--color-prime-primary)]" data-add-standard="${part.id}">+ Tambah Standard</button></div>${partStandards.map((standard, index) => `<div class="mb-3 rounded-lg border border-[var(--color-prime-border)] p-3"><div class="mb-2 grid gap-2 md:grid-cols-2"><input type="text" value="${standard.name ?? ''}" placeholder="Nama Standard" class="rounded-lg border border-[var(--color-prime-border)] px-3 py-2" data-standard-name="${part.id}:${index}"><select class="rounded-lg border border-[var(--color-prime-border)] px-3 py-2" data-standard-type="${part.id}:${index}"><option value="">Pilih Tipe...</option><option value="action" ${standard.input_type === 'action' ? 'selected' : ''}>Action (Pilihan)</option><option value="number" ${standard.input_type === 'number' ? 'selected' : ''}>Number (Target)</option><option value="range" ${standard.input_type === 'range' ? 'selected' : ''}>Range (Min-Max)</option></select></div><div class="mb-2 flex items-center gap-3 text-sm"><label><input type="checkbox" ${standard.is_required ? 'checked' : ''} data-standard-required="${part.id}:${index}"> Wajib Diisi</label><label><input type="checkbox" ${standard.is_active ? 'checked' : ''} data-standard-active="${part.id}:${index}"> Aktif</label><button type="button" class="ml-auto text-[var(--color-prime-danger)]" data-remove-standard="${part.id}:${index}">Hapus</button></div>${standard.input_type === 'action' ? `<div><div class="mb-2 flex gap-2"><input type="text" class="flex-1 rounded-lg border border-[var(--color-prime-border)] px-3 py-2" placeholder="Tambah action option" data-action-input="${part.id}:${index}"><button type="button" class="rounded-lg bg-[var(--color-prime-primary)] px-3 py-2 text-sm font-semibold text-white" data-add-action="${part.id}:${index}">Tambah</button></div><div class="flex flex-wrap gap-2">${(standard.action_options ?? []).map((option, optionIndex) => `<button type="button" class="rounded-full bg-[var(--color-prime-primary-soft)] px-3 py-1 text-xs text-[var(--color-prime-primary)]" data-remove-action="${part.id}:${index}:${optionIndex}">${option} x</button>`).join('')}</div></div>` : ''}${standard.input_type === 'number' ? `<div class="grid gap-2 md:grid-cols-2"><input type="number" step="0.01" value="${standard.target_value ?? ''}" placeholder="Target Value" class="rounded-lg border border-[var(--color-prime-border)] px-3 py-2" data-standard-target="${part.id}:${index}"><input type="text" value="${standard.unit ?? ''}" placeholder="Satuan" class="rounded-lg border border-[var(--color-prime-border)] px-3 py-2" data-standard-unit="${part.id}:${index}"></div>` : ''}${standard.input_type === 'range' ? `<div class="grid gap-2 md:grid-cols-3"><input type="number" step="0.01" value="${standard.min_value ?? ''}" placeholder="Min Value" class="rounded-lg border border-[var(--color-prime-border)] px-3 py-2" data-standard-min="${part.id}:${index}"><input type="number" step="0.01" value="${standard.max_value ?? ''}" placeholder="Max Value" class="rounded-lg border border-[var(--color-prime-border)] px-3 py-2" data-standard-max="${part.id}:${index}"><input type="text" value="${standard.unit ?? ''}" placeholder="Satuan" class="rounded-lg border border-[var(--color-prime-border)] px-3 py-2" data-standard-unit="${part.id}:${index}"></div>` : ''}</div>`).join('')}</div>`);
                });
            });
        ...
        const renderReview = () => {
            ...
            reviewBox.innerHTML = `<div class="grid gap-4 md:grid-cols-2"><div><p class="text-sm text-[var(--color-prime-muted)]">Mesin</p><p class="font-semibold">${machineNames || '-'}</p><p class="mt-3 text-sm text-[var(--color-prime-muted)]">Part</p><p class="font-semibold">${partCount} Part</p><p class="mt-3 text-sm text-[var(--color-prime-muted)]">Standard</p><p class="font-semibold">${standardCount} Standard</p></div><div><p class="text-sm text-[var(--color-prime-muted)]">Frekuensi</p><p class="font-semibold">${state.schedule.frequency_type || '-'}</p><p class="mt-3 text-sm text-[var(--color-prime-muted)]">Mulai</p><p class="font-semibold">${state.schedule.start_date || '-'}</p><p class="mt-3 text-sm text-[var(--color-prime-muted)]">Sampai</p><p class="font-semibold">${state.schedule.generate_until || '-'}</p></div></div>`;
        };
        ...
        if (machineLocationFilter) {
            ...
            machineLocationFilter.innerHTML = ['<option value="">Semua Lokasi</option>', ...locations.map((location) => `<option value="${location}">${location}</option>`)].join('');
        }
        renderMachineList();
        renderParts();
        renderStandards();
